...
function addStack(){
  ...
  // 정답 확정 후 추가 힌트는 스택/등급/점수에 영향을 주지 않음
  if(questionResolved){
    return;
  }
...
/* =========================================================
   정답 확정 후 미공개 힌트 활성화
========================================================= */
function enableUnrevealedHints(){
  ...
  $$("[data-hint]").forEach(button=>{
    ...
    if(type==="genres"){
      button.disabled = !(current && genreIndex < (current.genres||[]).length);
      return;
    }

    if(type==="cg"){
      const shots=(current && current.cgs||[]).filter(
        x=>(typeof x.sexual==="number"?x.sexual:0)<0.5 &&
           (typeof x.violence==="number"?x.violence:0)<1.5
      );
      button.disabled = !(current && cgIndex < shots.length);
      return;
    }

    button.disabled = used.has(type);
  });

  $$('[data-staff]').forEach(button=>{
    button.disabled = staffUsed.has(button.dataset.staff);
  });
}
...
function giveUp(){
  ...
  // addStack()이 questionResolved를 확인하므로 스택/등급/점수는 유지됨.
  questionResolved=true;
  enableUnrevealedHints();
